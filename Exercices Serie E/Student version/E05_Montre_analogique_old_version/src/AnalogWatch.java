...
public class AnalogWatch {
    ...
    public void paintComponent(Graphics g, int w, int h) {
        ...
        Point vec1; //major ticks
        Point vec2; //minor ticks
        ...
        {
            //vec1 et vec2 commencent au centre du cadran
            //vec1 sert à trouver le point de départ de la graduation
            //vec2 sert à trouver le point d'arrivée de la graduation 
            //vec2 se termine toujours sur la bordure du cadran
            angle = (i / (double)numberOfTicks) * 2*Math.PI;
            vec2 = polar2cart(center, radius, angle); 
            if (i % majorTick == 0)
                vec1 = polar2cart(center, radius-lengthMajorTick, angle);
            else
                vec1 = polar2cart(center, radius-lengthTick, angle);
            g.drawLine(vec1.x, vec1.y, vec2.x, vec2.y);
        }
        //***********************draw date**************************************
        g.setColor(Color.BLUE);
