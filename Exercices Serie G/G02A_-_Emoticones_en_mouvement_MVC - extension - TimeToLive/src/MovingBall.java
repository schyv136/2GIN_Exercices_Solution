 ...
 * La classe elle-même connaît les dimensions de son environnement.
 * Elle sait donc se déplacer elle-même.
 * Au lieu de step, xDir et yDir, elle possède deux attributs xStep et yStep 
 * du type double qui définissent ses pas en horizontale et en verticale.
 * 
...
public class MovingBall 
{
	...
	public void doStep(int width, int height)
	{
        ...
        if(x+xStep + radius > width)   xStep = - Math.abs(xStep);            
        if(x+xStep - radius < 0)       xStep =   Math.abs(xStep);            
        if(y+yStep + radius > height ) yStep = - Math.abs(yStep);
        if(y+yStep - radius < 0)       yStep =   Math.abs(yStep);
